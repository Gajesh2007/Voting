...
pub mod voting {
    ...
    pub fn create_proposal(
        ...
        for (index, proposal) in base_account.proposals.iter().enumerate() {
            if proposal.id == next_proposal_id {
                ...
                base_account.proposals[index].proposal = Proposal {
                    id: next_proposal_id,
                    proposal: name,
                    description: desc,
                    end_time_stamp: Clock::get()?.unix_timestamp as u128 + offset,
    ...
    pub fn vote(ctx: Context<VoteContext>, proposal_id: u128, choice_id: u128) -> ProgramResult {
        let base_account = &mut ctx.accounts.base_account;
        ...
        assert!(
            (Clock::get()?.unix_timestamp as u128)
